...
#
# TSB-AD Note: [This function is adapted from [TimeEval-algorithms] by [CodeLionX&wenig] - Original source: [https://github.com/TimeEval/TimeEval-algorithms]]
#
...
from TSB_AD.utils.utility import standardizer  
...
class KMeansAD(BaseEstimator, OutlierMixin):
    ...
    def _preprocess_data(self, X: np.ndarray, fit_scaler = False) -> np.ndarray:
        ...
        if self.normalize:# modified
            if fit_scaler:
                slides, self.scaler_ = standardizer(slides, keep_scalar=True)
            else:
                slides = self.scaler_.transform(slides)
    ...
    def fit(self, X: np.ndarray, y=None, preprocess=True) -> 'KMeansAD':
        if preprocess:
            X = self._preprocess_data(X, fit_scaler = True)
        self.model.fit(X)
        return self

    def decision_function(self, X: np.ndarray, preprocess=True) -> np.ndarray:
        if preprocess:
            X = self._preprocess_data(X, fit_scaler = False)
    ...
    def fit_predict(self, X, y=None) -> np.ndarray:
        ...
        return self.decision_function(X, preprocess=False)