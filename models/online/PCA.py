...
# - Deactivated pruning for streaming purposes.
...
from TSB_AD.utils.utility import check_parameter
...
class PCA(BaseDetector):
    ...
    def __init__(self, slidingWindow=100, sub = True, n_components=None, n_selected_components=None,
                 ...
                 weighted=True, standardization=True, zero_pruning=False):  # modified, zero_pruning = True by default
    ...
    def fit(self, X, y=None):
        ...
        if self.standardization:
            X, self.scaler_ = standardizer(X, keep_scalar=True) # Modified
    ...
    def decision_function(self, X):
        ...
        X = check_array(X)
        if self.standardization:
            X = self.scaler_.transform(X) # Modified
