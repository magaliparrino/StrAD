...
#
# --- ADAPTATION NOTICE ---
# This file is adapted from TSB-AD (https://github.com/thedatumorg/TSB-AD)
# Original License: Apache License 2.0
##
# MODIFICATIONS IN StrAD:
# - Stateful Feature-wise Scaling: Replaced window-wise standardization with 
#   feature-wise standardization.
# - Consistent Inference Pipeline: Updated decision_function() to apply training-phase 
#   scaling to test data, ensuring mathematical consistency between train and test.
...
from utils.dataset_stream import ForecastDataset    
...
class LSTMAD():
    ...
    def fit(self, data):
        ...
        if int(data.shape[0]) - (int((1-self.validation_size)*len(data))) < self.window_size + self.pred_len:
            tsTrain = data[:int(data.shape[0] - self.window_size - self.pred_len)]
            tsValid = data[int(data.shape[0] - self.window_size - self.pred_len):]
        else:
    ...
    def decision_function(self, data):
        ...
        test_loader = DataLoader(
            ForecastDataset(data_normalized, window_size=self.window_size, pred_len=self.pred_len, normalize=False),
            batch_size=self.batch_size,
            shuffle=False
        )
        #############################################
