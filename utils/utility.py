...
#
# TSB-AD Note: [A set of utility functions to support outlier detection.]
#
# MODIFICATIONS IN StrAD:
# - Added get_optimal_n_bins and ZNormalizer
...
class ZNormalizer:
    ...
    def fit_transform(self, X):
        """Computes mean and std then stadardize data."""
        X = np.asarray(X)
        self.mean_ = np.mean(X, axis=0)
        self.std_ = np.std(X, axis=0)
        self.std_ = np.where(self.std_ == 0, 1e-8, self.std_)

        return (X - self.mean_) if self.center_only else (X - self.mean_) / self.std_

    def transform(self, X):
        """Standardize new data by applying computed statistics."""
        if self.mean_ is None or self.std_ is None:
            raise ValueError("fit_transform mus be called before transform.")
        X = np.asarray(X)
        return (X - self.mean_) if self.center_only else (X - self.mean_) / self.std_
