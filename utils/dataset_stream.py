...
# - Adapted utils.dataset for streaming context
...
class ReconstructDataset(torch.utils.data.Dataset):
    def __init__(self, data, window_size, stride=1, normalize=True, center_only = False):
        super().__init__()
        ...
        if normalize:
            if center_only:
                self.data = self._center_data(data)
            else:
