...
def find_length_rank_for_multi(data, rank):
    n_features = data.shape[1]
    slidingWindow_list = []
    for i in range(n_features):
        slidingWindow_list.append(find_length_rank(data[:,i], rank=rank)) #.reshape(-1, 1) not necessary ?
    slidingWindow = np.max(slidingWindow_list)
